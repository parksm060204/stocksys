...
export default function CommoditySeasonalityPanel({
  ...
  if (!seasonality || seasonality.period <= 0) {
    return (
      <div className="flex flex-col h-full bg-[#0E1117] border border-border rounded-2xl p-5 font-mono text-xs shadow-xl">
        <div className="border-b border-border pb-2.5 flex items-center justify-between">
          ...
          <span className="text-[10px] text-[#8E939D] bg-[#161B22] px-2 py-0.5 rounded border border-border">
            연중 비계절 상품
          </span>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center text-center p-6 text-[#8E939D] space-y-2">
          ...
          <p className="text-xs text-[#565A63]">거시경제 지표, 지정학적 이슈, 재고 및 수급 압력에 주로 반응합니다.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-[#0E1117] border border-border rounded-2xl p-5 font-mono text-xs shadow-xl space-y-3.5 select-none">
      <div className="border-b border-border pb-2.5 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-base">📅</span>
        ...
        <span
          className={`text-[10.5px] font-bold px-2.5 py-0.5 rounded-full border ${
            isHighSeason
              ...
              : 'bg-[#161B22] text-[#8E939D] border-border'
          }`}
        >
      ...
      {/* SVG 그래프 */}
      <div className="relative bg-[#05070A] rounded-xl border border-border/60 p-2 overflow-hidden flex items-center justify-center">
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-[#161B22] p-2.5 rounded-xl border border-border/60">
        ...
        <div className="bg-[#161B22] p-2.5 rounded-xl border border-border/60">
          ...
          <div className={`font-black ${currentVal >= 0 ? 'text-up' : 'text-down'}`}>
            {currentVal >= 0 ? '+' : ''}
