  ...
  return (
    <div
      ...
      className={`p-4 rounded-2xl border cursor-pointer transition-all flex flex-col justify-between space-y-3 select-none font-mono ${
        isSelected
          ? 'border-[#F04452] bg-up/10 text-white shadow-[0_0_20px_rgba(240,68,82,0.2)]'
          : 'border-border bg-[#0E1117] text-[#8E939D] hover:border-white/20 hover:bg-[#161B22]'
      }`}
    >
      {/* Top Header & Badges */}
      <div className="flex justify-between items-start gap-1">
          ...
          <div className="flex items-center gap-2">
            ...
            {isLeverage && (
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${
                etf.leverageFactor > 0 ? 'bg-up/15 text-up border border-[#F04452]/30' : 'bg-down/15 text-down border border-[#3182F6]/30'
              }`}>
        ...
        <span className={`px-2.5 py-0.5 rounded-full text-[10.5px] font-extrabold border tabular-nums ${
          isPremium
            ? 'bg-up/15 text-up border-[#F04452]/30'
            : 'bg-down/15 text-down border-[#3182F6]/30'
        }`}>
      ...
      <div className="text-[10.5px] text-[#8E939D] font-medium truncate flex items-center gap-1.5 bg-[#05070A]/60 px-2.5 py-1 rounded-lg border border-border/60">
        <span className="text-down font-bold shrink-0">주요 비중:</span>
      ...
      <div className="grid grid-cols-2 gap-2 pt-2 border-t border-border text-xs">
        ...
        <div>
          ...
          <span className="font-black text-up tabular-nums text-[13.5px]">
            {formattedINAV}
          </span>
        </div>
      </div>
