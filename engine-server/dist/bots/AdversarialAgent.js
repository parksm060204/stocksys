...
class AdversarialAgent extends BaseAgent_1.BaseAgent {
    id = 'PROP_DESK_PREDATOR';
    lobHistory = new Map();
    constructor() {
        super({ id: 'PROP_DESK_PREDATOR', baseWeights: { stock: 0.5, bond: 0, commodity: 0, cash: 0.5 } }, 50000000000); // 500억
    }
    getTickSize(price) {
    ...
    executeManipulation(marketState) {
        const orders = [];
        // 단순 LOB 추론 로직
    ...
    executeFrontRunning(stockId, currentLOB) {
        const orders = [];
        ...
        if (history.length() === 5) {
            ...
            for (let i = 1; i < history.length(); i++) {
                ...
                if (curr.bestBidPrice !== basePrice)
                    priceStagnant = false;
...
//# sourceMappingURL=AdversarialAgent.js.map