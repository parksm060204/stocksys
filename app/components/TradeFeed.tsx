...
export const TradeFeed: React.FC<TradeFeedProps> = ({ stock, trades: externalTrades, rolloverEvents = [] }) => {
  ...
  return (
    <div className="flex flex-col h-full text-xs font-sans select-none bg-[#090B0F]">
      {/* 헤더 */}
      <div className="bg-[#090B0F] px-4 py-2 border-b border-[#1e2230] flex justify-between items-center shrink-0">
        <span className="text-[12.5px] font-black text-white flex items-center gap-2">
          <span>실시간 체결</span>
          <span className="text-[10px] font-mono text-[#8E939D] font-normal uppercase">Trade Feed</span>
        </span>
        <span
          className={`text-[10px] font-mono font-bold flex items-center gap-1.5 px-2 py-0.5 rounded-full border ${
            source === 'db' ? 'text-up bg-up/10 border-[#F04452]/30' : 'text-emerald-400 bg-emerald-400/10 border-emerald-400/30'
          }`}
        >
      ...
      <div className="flex-1 overflow-y-auto p-1.5 space-y-1 bg-[#05070A] scrollbar-thin scrollbar-thumb-gray-800">
        {/* 롤오버 특수 알림 */}
        {rolloverEvents.length > 0 && (
          <div className="p-2.5 bg-[#002b36] border border-[#005f73] rounded-xl text-xs text-[#005f73] animate-pulse space-y-0.5">
            ...
            <div className="text-[#8E939D] text-[10.5px]">
        ...
        {/* 체결 테이블 헤더 */}
        <div className="grid grid-cols-3 text-[10.5px] text-[#8E939D] font-bold px-3 py-1 border-b border-[#1e2230] bg-[#090B0F]/50 rounded-lg">
          <span>시간</span>
        ...
        {trades.length === 0 ? (
          ...
          trades.map((t) => {
            ...
            return (
              <div key={t.tradeId} className="grid grid-cols-3 items-center px-3 py-1 border-b border-[#1e2230]/40 text-[11.5px] hover:bg-[#141721]/60 transition-colors font-mono">
                <span className="text-[#8E939D] text-[10.5px] tabular-nums">
