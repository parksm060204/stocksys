...
async function testUpstashRedisRateLimiterIntegrity() {
  ...
  try {
    ...
    // 7. Invalid EXPIRE results and unexpiring TTL <= 0 defense -> 503, Gemini called 0 times
    const invalidExpireTtlCases = [
      { name: 'Unexpiring key {INCR:1, EXPIRE:0, TTL:-1}', resp: [{ result: 1 }, { result: 0 }, { result: -1 }] },
      { name: 'Expired key TTL:0', resp: [{ result: 1 }, { result: 1 }, { result: 0 }] },
      { name: 'Non-existent key TTL:-2', resp: [{ result: 1 }, { result: 1 }, { result: -2 }] },
      { name: 'Null EXPIRE result', resp: [{ result: 1 }, { result: null }, { result: 60 }] },
      { name: 'Arbitrary string EXPIRE result "ok"', resp: [{ result: 1 }, { result: 'ok' }, { result: 60 }] },
      { name: 'Out-of-range integer EXPIRE result 2', resp: [{ result: 1 }, { result: 2 }, { result: 60 }] },
    ];
    for (const { name, resp } of invalidExpireTtlCases) {
      mockPipelineResponse = resp;
      geminiCallCount = 0;
      const errRes = await analyzePostHandler(makeReq());
      assert.strictEqual(errRes.status, 503, `${name} must return 503 error`);
      assert.strictEqual(geminiCallCount, 0, `Gemini must NOT be called on ${name}`);
      await assert.rejects(
        async () => defaultUpstashStore.consume('test', 5, 60000),
        (err: Error) =>
          err instanceof RateLimiterServiceUnavailableError &&
          (err.message.includes('Invalid Redis TTL') || err.message.includes('Invalid Redis EXPIRE'))
      );
    }
    console.log('✅ [PASS] Unexpiring key (TTL <= 0) and invalid EXPIRE return 503 without defaulting to window');
