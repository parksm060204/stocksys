...
class EventDirector {
    ...
    async tickMinute() {
        ...
        // 시장 시간 외 무시
        if (hours < 18 || (hours === 22 && minutes > 30) || hours > 22)
            return;
    ...
    async generateCalendarEvent(calEvent) {
        try {
            const prompt = `
...
`;
    ...
    scheduleCorrection(fakeNewsData, allOutlets) {
        ...
        setTimeout(async () => {
            try {
                ...
                // 엔진에 반대 이벤트 주입
                const inverseImpact = (impact) => {
                    if (impact === 'STRONG_POSITIVE')
...
exports.EventDirector = EventDirector;
