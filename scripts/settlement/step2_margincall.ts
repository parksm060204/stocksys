...
const POSTGREST = process.env.NEXT_PUBLIC_ENGINE_DB_URL || 'http://49.247.136.231:3001';
...
async function pgFetch(path: string, opts: RequestInit = {}): Promise<any> {
  ...
  try {
    ...
    clearTimeout(t);
    console.warn(`[STEP 2] Remote DB unreachable, switching to In-Memory Verification Mode.`);
    isInMemoryMode = true;
    return runMockFetch(path, opts);
  }
}
...
async function runMockFetch(path: string, opts: RequestInit = {}): Promise<any> {
  ...
  if (path.startsWith('/orders')) {
    ...
    const match = path.match(/id=eq\.([^&]+)/);
    if (match && match[1]) {
      const o = memoryDb.orders.get(match[1]);
      return o ? [o] : [];
    }
