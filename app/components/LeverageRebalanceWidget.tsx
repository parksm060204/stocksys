...
export const LeverageRebalanceWidget: React.FC<LeverageRebalanceWidgetProps> = ({
  ...
  return (
    <div className="bg-[#0E1117] border border-border p-4 text-xs font-mono rounded-2xl text-white select-none shadow-xl">
      <div className="flex justify-between items-center mb-3 pb-2 border-b border-border">
        <span className="font-extrabold text-up flex items-center gap-1.5 text-[13px]">
          <span>{etfTicker} ({leverage > 0 ? `+${leverage}` : leverage}X)</span>
        </span>
        <span className="text-[10.5px] text-[#8E939D] font-bold">일단위 리밸런싱 현황</span>
      </div>
      
      <div className="space-y-2 mb-3 bg-[#05070A] p-3 rounded-xl border border-border">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-black text-up tabular-nums">
      ...
      {/* 리밸런싱 예상 알림 */}
      <div className="bg-[#161B22] p-2.5 border border-border rounded-xl flex justify-between items-center text-[11.5px]">
        <span className="text-[#8E939D] font-bold">장 마감 예상 리밸런싱:</span>
        <span className={`font-black tracking-wide tabular-nums ${isBuy ? 'text-up' : 'text-down'}`}>
