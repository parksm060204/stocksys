      ...
      <div className="mx-auto max-w-2xl px-6 py-20 text-center font-sans">
        <div className="rounded-2xl border border-border bg-[#141721] p-10 space-y-4 shadow-xl">
          ...
          <div className="pt-4">
            <Link
              href="/"
              className="inline-flex items-center justify-center px-6 py-3 rounded-xl bg-down hover:bg-[#2b72d6] text-white font-bold text-[13px] transition-all"
            >
              메인홈으로 돌아가기
            </Link>
          </div>
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-6 font-sans space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* 보유 종목 테이블 */}
        <div className="lg:col-span-2 rounded-3xl border border-border bg-[#0E1117] overflow-hidden shadow-2xl">
          <div className="border-b border-border px-5 py-4 flex items-center justify-between bg-[#090B0F]">
            <h2 className="text-[14px] font-black text-white">보유 종목 리스트</h2>
            <span className="text-xs text-[#8E939D] font-mono font-bold">{rows.length}개 종목</span>
          </div>
          <div className="overflow-x-auto">
            {rows.length === 0 ? (
              ...
              <table className="w-full text-left text-[13px] border-collapse font-mono">
                <thead className="border-b border-border text-xs font-extrabold uppercase tracking-wider text-[#8E939D] bg-[#090B0F]">
                  <tr>
                  ...
                  {rows.map((r: any) => (
                    <tr key={r.stockId} className="transition-colors hover:bg-black/[0.03] dark:hover:bg-white/[0.04] border-b border-border last:border-none">
                      <td className="px-5 py-4 border-none">
                        ...
                        <div className="font-mono text-xs text-[#565A63] font-bold">{r.ticker}</div>
                      </td>
                      ...
                      <td className={`px-5 py-4 text-right font-mono tabular-nums font-black ${r.pnl >= 0 ? "text-up" : "text-down"}`}>
                        {r.pnl >= 0 ? "+" : ""}{fmtPrice(r.pnl, r.market as any)}
                      </td>
                      <td className={`px-5 py-4 text-right font-mono font-black tabular-nums ${r.pnlPct >= 0 ? "text-up" : "text-down"}`}>
                        <span className={`inline-block px-2 py-0.5 rounded-full border text-[11.5px] ${
                          r.pnlPct >= 0 ? "bg-up/10 border-[#F04452]/30 text-up" : "bg-down/10 border-[#3182F6]/30 text-down"
                        }`}>
        ...
        {/* 보유 외화 지갑 카드 */}
        <div className="rounded-3xl border border-border bg-[#0E1117] p-5 flex flex-col shadow-2xl space-y-4">
...
function Summary({ label, value, tone }: { label: string; value: string; tone?: "up" | "down" }) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-[#0E1117] p-4 shadow-xl">
...
function CurrencyItem({ code, name, amount, icon }: { code: string; name: string; amount: number; icon: string }) {
  return (
    <div className="rounded-2xl border border-border bg-[#05070A] p-3.5 hover:border-[#F04452]/40 transition-all font-mono">
