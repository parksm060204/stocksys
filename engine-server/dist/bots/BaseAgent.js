...
class BaseAgent {
    ...
    agentConfig;
    currentPortfolio;
    constructor(configOrId, initialCapital) {
        ...
        const weights = { ...(this.agentConfig.targetAllocation || this.agentConfig.baseWeights || { stock: 0, bond: 0, commodity: 0, cash: 1, kr_equity: 0, us_equity: 0, eu_equity: 0, derivatives: 0 }) };
     ...
     */
    calculateTargetWeights(sentiment, activeEvents) {
        let target = { ...(this.agentConfig.targetAllocation || this.agentConfig.baseWeights || { stock: 0, bond: 0, commodity: 0, cash: 1 }) };
        ...
        if (this.agentConfig.riskTolerance > 1.0) {
            ...
            if (totalEquities > 0) {
                if (target.stock > 0)
                    target.stock += boost;
                if (target.kr_equity > 0)
                    target.kr_equity += boost * (target.kr_equity / totalEquities);
                if (target.us_equity > 0)
                    target.us_equity += boost * (target.us_equity / totalEquities);
                if (target.eu_equity > 0)
                    target.eu_equity += boost * (target.eu_equity / totalEquities);
            }
            if (target.commodity > 0)
                target.commodity += boost;
            target.bond -= boost;
            target.cash -= boost;
        }
        ...
        if (total > 0) {
            ...
            target.cash /= total;
            if (target.kr_equity)
                target.kr_equity /= total;
            if (target.us_equity)
                target.us_equity /= total;
            if (target.eu_equity)
                target.eu_equity /= total;
        }
        return target;
    }
    /**
     ...
     */
    executePortfolioRebalancing(marketState) {
        const orders = [];
        ...
        if (this.agentConfig.executionStyle === 'AGGRESSIVE_MARKET')
            twapRatio = 0.20; // 20%씩 공격적
        if (this.agentConfig.executionStyle === 'HFT_LIMIT')
            twapRatio = 0.10; // 10%
        if (this.agentConfig.executionStyle === 'PASSIVE_TWAP')
            twapRatio = 0.02; // 2%씩 천천히
        // 5. 종목 선정 및 주문 생성
        // 주식 거래
     ...
     */
    executeSmartOrder(stock, side, targetPrice, targetQty, baseUrgency, activeEvents = []) {
