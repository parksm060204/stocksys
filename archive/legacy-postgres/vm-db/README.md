...
Legacy DB 이그레스 초과로 VM 자체 DB 로 이관하기 위한 산출물 모음.

## 구성

- **PostgreSQL 16** (Docker, 볼륨 영속)
- **PostgREST 12** — Legacy DB SDK 호환 REST API
- **Adminer 4** (관리 UI, SSH 터널 전용)

> Legacy DB Self-Host 전체 스택(GoTrue/Realtime/Postgres 메타)이 아닌, **Postgres + PostgREST** 만 올립니다.
...
└── sql/
    ...
    │   ├── 01_schema.sql         # 스키마 (Legacy DB 호환 함수 포함)
...
```env
NEXT_PUBLIC_ENGINE_DB_URL=http://VM_IP:3001
NEXT_PUBLIC_ENGINE_DB_ANON_KEY=eyJhbGc...        # anon JWT
ENGINE_DB_SERVICE_ROLE_KEY=eyJhbGc...           # service_role JWT (백엔드만)
```
...
| 기능                | 상태       | 대응                                              |
|---------------------|------------|---------------------------------------------------|
| 데이터 CRUD          | 정상       | PostgREST 가 Legacy DB SDK REST 와 호환             |
...
- **Realtime 재도입**: 
  - 옵션 A) Legacy DB Self-Host 의 Realtime 만 추가 (Elixir 서비스)
